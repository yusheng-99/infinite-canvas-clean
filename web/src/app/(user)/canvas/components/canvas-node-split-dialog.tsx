...
export function CanvasNodeSplitDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (params: CanvasImageSplitParams) => void }) {
    ...
    const addLine = (axis: "horizontal" | "vertical") => setParams((current) => {
    ...
    const setLine = (axis: "horizontal" | "vertical", index: number, value: number) => setParams((current) => {
        const key = axis === "horizontal" ? "horizontalLines" : "verticalLines";
        const lines = [...(current[key] || [])];
        lines[index] = Math.min((lines[index + 1] ?? 1) - 0.01, Math.max((lines[index - 1] ?? 0) + 0.01, value));
    ...
    const startDrag = (axis: "horizontal" | "vertical", index: number, event: ReactPointerEvent) => {
        ...
        if (!box) return;
        const move = (next: PointerEvent) => setLine(axis, index, axis === "horizontal" ? (next.clientY - box.top) / box.height : (next.clientX - box.left) / box.width);
                ...
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_280px]">
                    ...
                    <div className="space-y-5 py-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <Button icon={<ListRestart className="size-4" />} onClick={() => setParams((current) => ({ ...current, horizontalLines: buildGridLines(rows), verticalLines: buildGridLines(columns) }))}>重置线</Button>
                        </div>
                        ...
                        <Button type="primary" size="large" className="w-full" icon={<Grid2x2 className="size-4" />} onClick={() => onConfirm({ ...params, rows, columns, horizontalLines, verticalLines })}>
                            生成子节点
                        </Button>
                    </div>
                </div>
...
function findLineSpot(lines: number[]) {
    ...
    for (let index = 0; index < cuts.length - 1; index += 1) {
        const next = cuts[index + 1] - cuts[index];
        if (next > gap) { gap = next; spot = cuts[index] + next / 2; }
    }
    return spot;
}
